...
{
  uint32_t len = tail > head ? tail - head : inl(AUDIO_SBUF_SIZE_ADDR) + tail - head;
  return len;
}

static uint32_t ring_queue_add(uint32_t ptr, uint32_t num)
{
  uint32_t new_ptr = ptr + num >= inl(AUDIO_SBUF_SIZE_ADDR) ? ptr + num - inl(AUDIO_SBUF_SIZE_ADDR) : ptr + num;
  return new_ptr;
}
...
void __am_audio_play(AM_AUDIO_PLAY_T *ctl) {
  ...
  while (count + audio_len >= buf_size)
  {
    head = inw(AUDIO_COUNT_ADDR + 2);
    count = ring_queue_len(head, tail);//wait for play
  }
